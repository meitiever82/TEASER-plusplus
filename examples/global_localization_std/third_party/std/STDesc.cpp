...
// VENDOR-MOD: replaced ROS-based read_parameters with a non-ROS overload that
// sets the same defaults the original would have used when no ROS params are set.
// Callers can mutate fields on the returned struct directly to override.
void init_default_parameters(ConfigSetting &config_setting) {
  // pre-process
  config_setting.ds_size_ = 0.5;
  ...
  config_setting.proj_dis_max_ = 2;
  ...
  config_setting.icp_threshold_ = 0.5;
...
// VENDOR-MOD: publish_std_pairs (rviz visualization) removed - it pulled in
// visualization_msgs / ros::Publisher dependencies that are not needed for
// place recognition. Add it back in a separate visualization shim if needed.
...
void STDescManager::SearchLoop(
  ...
  if (stds_vec.size() == 0) {
    std::cerr << "[STD] No STDescs!" << std::endl;
