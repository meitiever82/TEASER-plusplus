...
// Vendored from https://github.com/hku-mars/STD (std_detector / Yuan et al., ICRA 2023).
// Stripped of ROS dependencies (parameter loading, rviz publisher, ROS_* macros)
// and patched for Ceres < 2.1 (Manifold -> LocalParameterization).
...
// VENDOR-MOD: replaces read_parameters(ros::NodeHandle&, ...). Sets defaults that
// match the original ROS yaml-less defaults. Mutate fields after the call to override.
...
// VENDOR-MOD: publish_std_pairs (rviz visualization) removed; not needed for
// non-ROS place recognition.
